...
<div
	class="opacity-80 dark:opacity-90 hover:opacity-100 transition-all w-80 h-36 w-full rounded-xl dark:bg-gradient-to-br bg-gradient-to-br dark:from-slate-600 from-gray-100 dark:via-gray-800 via-gray-100 dark:to-slate-700 to-slate-200 p-[1px]"
>
	<a href="/records/{record.id}">
		...
		>
			<!-- Logo -->
			<div class="flex">
				<Toggle checked={record.enabled === 1} />
				<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 256 116"
					...
					/></svg
				>
				<h1 class="ml-4 font-semibold mt-1">{record.name} &#8226; {record.type}</h1>
			</div>
			...
			<div class="flex items-center justify-between mt-4">
				<div class="flex space-x-6 text-sm dark:text-gray-200">
					<!-- <div>
            <p class="text-xs opacity-50">Record Content:</p>
            <p>{record.content}</p>
        </div> -->
					<div>
						<p class="text-xs opacity-70 dark:opacity-50">Last Modified</p>
						<p>{record.modified_on}</p>
					</div>
				</div>
				<svg
